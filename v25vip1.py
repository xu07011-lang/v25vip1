        ...
        try:
            ...
            if latest_issue_id != session_state["last_known_issue_id"]:
                ...
                if last_bet and str(last_bet["issue_id"]) == latest_issue_id:
                    ...
                    # --- ĐÃ SỬA (LOGIC CƯỢC 1 LẦN) ---
                    # Logic cũ dùng 2 biến cược. Logic mới chỉ dùng 1.
                    # last_bet['amount'] bây giờ là TỔNG TIỀN CƯỢC
                    bet_room, total_bet_for_round, balance_before = last_bet['room'], last_bet['amount'], session_state['balance_before_bet']
                    ...
                    if is_win:
                        ...
                        # --- ĐÃ SỬA (LOGIC CƯỢC 1 LẦN) ---
                        # Logic tính lợi nhuận cũ: (initial_bet_amount_from_last_round * 0.95) - final_bet_amount_from_last_round
                        # Logic mới (cược 1 lần): (tổng cược * 0.95)
                        round_profit = (final_balance - balance_before) if final_balance is not None and balance_before is not None else (total_bet_for_round * 0.95)
                        # --- HẾT SỬA ---
                    else: # THUA
                        ...
                        session_state["current_bet_amount"] = session_state["last_initial_bet_amount"] * MULTIPLIER # Vẫn dùng last_initial_bet_amount để tính cược ván sau
                        
                        # --- ĐÃ SỬA (LOGIC CƯỢC 1 LẦN) ---
                        # Logic lỗ cũ: -(initial_bet_amount_from_last_round + final_bet_amount_from_last_round)
                        # Logic mới (cược 1 lần): -tổng cược
                    ...
                    # --- ĐÃ SỬA (LOGIC CƯỢC 1 LẦN) ---
                    action_text = Text(f"Cược: Phòng {bet_room} ({ROOM_NAMES.get(str(bet_room), '?')})", style=STYLE_INFO) # Sửa "Cược cuối" -> "Cược"
                    # --- HẾT SỬA ---
                ...
                # --- CƯỢC LẦN 1: CƯỢC BAN ĐẦU (ĐÂY LÀ CƯỢC DUY NHẤT) ---
                ...
                console.print(f"🎯 [bold]Cược:[/bold] Cược [bold blue]{bet_amount:.4f} {BET_TYPE}[/bold blue] vào phòng [bold blue]{initial_room} ({ROOM_NAMES.get(str(initial_room), '?')})[/bold blue] cho ván [bold]#{next_round_id}[/bold]...")
                ...
                if not (bet_response and bet_response.get("code") == 0):
                    console.print(f"❌ Cược thất bại! Phản hồi: {bet_response}", style="red"); session_state["last_bet_on"] = None; continue
                
                console.print("✅ Cược thành công.", style="green")
                # Gán cược này là cược cuối cùng để vòng lặp sau xử lý
                session_state["last_bet_on"] = {"issue_id": next_round_id, "room": initial_room, "amount": bet_amount}

                # --- CƯỢC LẦN 2: ĐÃ BỊ XÓA THEO YÊU CẦU ---
                # Toàn bộ khối logic Cược lần 2, bao gồm sleep, random, và gọi API lần 2 đã được loại bỏ.
